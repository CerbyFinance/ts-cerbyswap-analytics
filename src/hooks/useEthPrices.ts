...
export function useCerbyPrices(): number | undefined {
  const [prices, setPrices] = useState<{ [network: string]: number | undefined }>()
  const [error, setError] = useState(false)
  const { dataClient } = useClients()
  ...
  useEffect(() => {
    async function fetchPrice() {
      await fetch("https://supply.cerby.fi/cerby/supply-marketcap")
        .then(async r => {
          const response = await r.json();
          setPrices({
            [activeNetwork.id]: response.data.currentWeightedPrice,
          })
        })
        .catch(setError.bind(null, true));
    }
    if (!indexedPrices && !error && formattedBlocks) {
      fetchPrice()
    }
